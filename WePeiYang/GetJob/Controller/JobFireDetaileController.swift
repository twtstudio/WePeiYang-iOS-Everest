...
class JobFireDetaileController: UIViewController {
    ...
    func setUIKit() {
        ...
        JobFireDetailUrl.url = RecruitmentUrl
        JobFireDetailHelper.getJobFireDetail(success: { jobFireDetail in
            self.titleLable.text = (jobFireDetail.data?.title)!
            self.visitsLable.text = String((jobFireDetail.data?.click)!)
            self.timeLable.text = (jobFireDetail.data?.date)!
            
            
            let content = (jobFireDetail.data?.content)!
            ...
            self.fireTimeLable.text = "\((jobFireDetail.data?.heldDate)!)  \((jobFireDetail.data?.heldTime)!)"
            ...
            self.locateLable.text = (jobFireDetail.data?.place)!
            self.scrollView.addSubview(self.locateLable)
            
            if jobFireDetail.data?.attach1 != nil {
                self.attachArray.append((jobFireDetail.data?.attach1)!)
                ...
                self.fileBtn1.setTitle((jobFireDetail.data?.attach1Name)!, for: .normal)
            ...
            if jobFireDetail.data?.attach2 != nil {
                self.attachArray.append((jobFireDetail.data?.attach2)!)
                ...
                self.fileBtn2.setTitle((jobFireDetail.data?.attach2Name)!, for: .normal)
            ...
            if jobFireDetail.data?.attach3 != nil {
                self.attachArray.append((jobFireDetail.data?.attach3)!)
                ...
                self.fileBtn3.setTitle((jobFireDetail.data?.attach3Name)!, for: .normal)
