...
class JNStarRateView: UIView {
    ...
    init(frame: CGRect,starCount: Int, score: Float) {
        //        super.init(frame: CGRect(x: frame.minX, y: frame.minY, width: frame.width + CGFloat(score * 2), height: frame.height))
    ...
    fileprivate func createStarView()->(){
        starBackgroundView = starViewWithImageName(#imageLiteral(resourceName: "icon_心_n"))
        starForegroundView = starViewWithImageName(#imageLiteral(resourceName: "icon_心_s"))
    ...
    fileprivate func starViewWithImageName(_ image: UIImage) -> UIView {
        let starView = UIView.init(frame: self.bounds)
        starView.clipsToBounds = true
        //添加星星
        let width = (self.frame.size.width - CGFloat(5 * 2)) / CGFloat(count)
        for index in 0...count {
            let imageView = UIImageView.init(frame: CGRect(x: CGFloat(index) * (width + 2),y: 0, width: width, height: self.bounds.size.height))
