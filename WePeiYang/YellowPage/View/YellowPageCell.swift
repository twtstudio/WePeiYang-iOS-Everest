...
class YellowPageCell: UITableViewCell {
    ...
    convenience init(with style: YellowPageCellStyle, model: UnitList) {
        ...
        phoneView.setImage(UIImage(named: "itemPhone"), for: .normal)
    ...
    @objc func longPressed(sender: UITapGestureRecognizer) {
        if let text = UIPasteboard.general.string, text != self.detailedModel.itemPhone {
            UIPasteboard.general.string = self.detailedModel.itemPhone
        }
