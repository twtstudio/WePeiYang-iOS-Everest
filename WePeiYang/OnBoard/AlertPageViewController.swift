...
protocol AlertPageViewDelegate: class {
...
class AlertPageViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    weak var delegate: AlertPageViewDelegate?
    ...
    func pageViewController(_ pageViewController: UIPageViewController, didFinishAnimating finished: Bool, previousViewControllers: [UIViewController], transitionCompleted completed: Bool) {
        ...
        if pageControl != nil {
            ...
            if pageControl.currentPage == arrayOfImage.count - 1 {
                self.alertview.buttonBottom.setTitle(alertview.titleGotItButton, for: [.normal, .highlighted, .disabled, .selected, .focused, .application, .reserved])
            } else {
                self.alertview.buttonBottom.setTitle(alertview.titleSkipButton, for: [.normal, .highlighted, .disabled, .selected, .focused, .application, .reserved])
            }
        }
    }
    ...
    fileprivate func configurePageViewController() {
        ...
        let initialViewController = self.viewController(at: arrayOfImage.count-1)
